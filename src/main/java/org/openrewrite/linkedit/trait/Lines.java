 ...
 * root, because every statement of a deck asks.
 */
@NoArgsConstructor(access = AccessLevel.PRIVATE)
final class Lines {

    static Map<UUID, Integer> of(Cursor cursor) {
        LinkEdit.CompilationUnit cu = cursor.firstEnclosingOrThrow(LinkEdit.CompilationUnit.class);
        return cursor.getRoot().computeMessageIfAbsent("linkEdit.lines." + cu.getId(), k -> {
            Map<UUID, Integer> lines = new HashMap<>();
            new LinkEditIsoVisitor<Map<UUID, Integer>>() {
                int line = 1;
                ...
                @Override
                public LinkEdit.Word visitWord(LinkEdit.Word word, Map<UUID, Integer> p) {
                    LinkEdit.Word w = super.visitWord(word, p);
                    p.put(w.getId(), line);
                    return w;
                }
            }.visit(cu, lines);
