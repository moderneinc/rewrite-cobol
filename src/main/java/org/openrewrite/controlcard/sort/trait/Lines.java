 ...
 * the words are separated by — the same text printing puts back. Counted once per deck and kept on
 * the cursor root, because every statement of a deck asks.
 */
@NoArgsConstructor(access = AccessLevel.PRIVATE)
final class Lines {

    static Map<UUID, Integer> of(Cursor cursor) {
        Sort.CompilationUnit cu = cursor.firstEnclosingOrThrow(Sort.CompilationUnit.class);
        return cursor.getRoot().computeMessageIfAbsent("sort.lines." + cu.getId(), k -> {
            Map<UUID, Integer> lines = new HashMap<>();
            new SortIsoVisitor<Map<UUID, Integer>>() {
                int line = 1;
                ...
                @Override
                public Sort.Word visitWord(Sort.Word word, Map<UUID, Integer> p) {
                    Sort.Word w = super.visitWord(word, p);
                    p.put(w.getId(), line);
                    return w;
                }
            }.visit(cu, lines);
